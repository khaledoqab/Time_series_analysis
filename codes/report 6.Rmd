...
```{r}
plot(t_vals, parameters, type='b', main="c", ylim = c(-0.03, 0.03)) +
...
for (i in 0:17){
    u = linear_coef(temperatures, 2000 + i, 5)
    parameters <- cbind(parameters, u[1])
    errors <- cbind(errors, u[2])
}
...
```{r}
plot(t_vals, parameters, type='b', main="c", ylim = c(-0.010, 0.010)) +
  arrows(x0=t_vals, y0=parameters-errors, x1 = t_vals, y1=parameters+errors, code=3, angle = 90, length = 0.1)
